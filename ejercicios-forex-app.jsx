...
function genArbitrajeSimple(live){
  ...
  return {
    ...
    context:`Son las 9:15 de la mañana en la sala de mercados de un banco de inversión europeo. En las pantallas de Bloomberg parpadean, simultáneamente, las cotizaciones spot del dólar estadounidense en dos plazas financieras: Frankfurt —donde opera el Sistema Europeo de Bancos Centrales— y Nueva York —sede de la Reserva Federal—. Ambas plazas están abiertas al mismo tiempo durante la franja horaria de solapamiento europeo-americano, que concentra cerca del 40% del volumen diario global de divisas (aprox. 2,7 billones de dólares según el BIS Triennial Survey). El operador de divisas detecta una discrepancia entre los tipos cotizados: el EUR/USD no es idéntico en las dos plazas. En un mercado perfectamente eficiente esta diferencia no debería existir —o desaparecería en milisegundos—, pero en este momento la ventana sigue abierta. Datos de mercado a fecha ${live.ratesDate||"hoy"} (fuente: Frankfurter API / BCE).`,
...
function genArbitrajeTriangular(live){
  ...
  return {
    ...
    context:`El mercado de divisas interbancario opera de forma descentralizada (OTC) con más de 30 pares de divisas cotizando simultáneamente las 24 horas. Cuando no existe un mercado directo y líquido entre dos divisas, los operadores utilizan el dólar estadounidense como moneda vehicular para calcular el tipo cruzado implícito. En este ejercicio, el departamento de tesorería de un banco europeo detecta, durante la sesión del ${live.ratesDate||"hoy"}, que el tipo de cambio cotizado directamente entre ${c1} y ${c3} difiere del tipo cruzado que se obtiene encadenando las operaciones a través del ${c2}. Esta incoherencia en los precios —aunque pequeña en términos porcentuales— puede generar beneficios sin riesgo si se opera con volúmenes suficientes. En los mercados modernos, los algoritmos de alta frecuencia (HFT) explotan estas ventanas en cuestión de microsegundos, por lo que el ejercicio representa una foto fija del momento en que la oportunidad está abierta.`,
...
function genArbitrajeCrypto(live){
  ...
  return {
    ...
    context:`El fenómeno conocido como "${exc.prem}" hace referencia a la prima persistente con la que Bitcoin cotiza en ${exc.e2} respecto a mercados internacionales como ${exc.e1}. A diferencia del mercado de divisas tradicional —donde el arbitraje se ejecuta en milisegundos—, el arbitraje entre exchanges de criptomonedas enfrenta fricciones reales: los controles de capital impiden mover libremente fondos entre jurisdicciones, los tiempos de confirmación en la blockchain pueden superar los 10 minutos, y los requisitos KYC/AML limitan los volúmenes operables. La prima puede mantenerse durante días o semanas precisamente porque estas fricciones impiden el arbitraje perfecto. El episodio más extremo del Kimchi Premium se registró en enero de 2018, cuando Bitcoin cotizaba hasta un 50% más caro en Corea del Sur que en Occidente, coincidiendo con restricciones gubernamentales a las cuentas bancarias de los exchanges. En este ejercicio, un inversor europeo observa una prima del ${(prem*100).toFixed(2)}% entre ${exc.e1} y ${exc.e2} y evalúa si es posible y rentable explotarla.`,
...
function genPCI(live){
  ...
  return {
    topicId:"t5_pci",
    context:`Un gestor de renta fija en una gestora de activos con sede en Fráncfort debe decidir cómo invertir ${fmtK(cap)} € de liquidez durante los próximos ${pl} meses. Tiene dos opciones: mantener el capital en un depósito en euros al ${rE}% anual, o convertirlo a dólares, invertirlo al ${rU}% anual en EE.UU. y asegurar el tipo de cambio de retorno mediante un contrato forward. La segunda estrategia, conocida como carry trade cubierto, elimina el riesgo cambiario y permite comparar rentabilidades en términos homogéneos. La Paridad Cubierta de Intereses (PCI) establece que, en equilibrio, ambas alternativas deben ofrecer el mismo rendimiento: si no fuera así, los operadores explotarían el diferencial hasta eliminar la discrepancia. Esta condición de no-arbitraje es uno de los pilares del mercado de divisas y determina el tipo forward de equilibrio. El BCE fija actualmente los tipos de referencia en la Eurozona en el ${rE}%, mientras la Fed mantiene los suyos en el ${rU}%.`,
...
function genPDI(live){
  ...
  return {
    topicId:"t5_pdi",
    context:`La Paridad Descubierta de Intereses (PDI) es la condición de equilibrio del mercado de divisas cuando los inversores no utilizan cobertura forward y asumen el riesgo cambiario. Actualmente, el diferencial de tipos de interés entre EE.UU. y la Eurozona es de ${diff.toFixed(2)} puntos porcentuales a favor del dólar (R$ = ${rU}% frente a R€ = ${rE}%). En principio, esto hace atractivo invertir en dólares —una lógica que subyace al carry trade—. Sin embargo, la PDI establece que este diferencial debería verse compensado exactamente por una depreciación esperada del dólar: si el mercado anticipa que el $ se depreciará un ${diff.toFixed(2)}%, la rentabilidad ajustada por riesgo cambiario es idéntica en ambas divisas. En la práctica, la PDI falla sistemáticamente a corto plazo —el denominado "forward premium puzzle"—, lo que explica la popularidad del carry trade como estrategia especulativa. Un inversor europeo evalúa si el diferencial de tipos justifica asumir riesgo cambiario apostando por el dólar.`,
...
function genPoliticaMonetaria(live){
  ...
  return {
    topicId:"t6_corto",
    context:`El Consejo de Gobierno del BCE acaba de anunciar una reducción de su tipo de interés de referencia del ${r0}% al ${r1}%, argumentando que la inflación de la Eurozona ha convergido al objetivo del 2% y que la actividad económica necesita estímulo. La Reserva Federal estadounidense, en cambio, mantiene sus tipos en el ${rF}% ante la persistencia de presiones inflacionistas en EE.UU. Este diferencial creciente entre la política monetaria del BCE y la Fed —un fenómeno conocido como divergencia de ciclos monetarios— tiene consecuencias directas sobre el tipo de cambio EUR/USD. A corto plazo, con precios rígidos (supuesto del modelo de Dornbusch), el ajuste recae íntegramente sobre el tipo de cambio nominal. El mecanismo de transmisión opera a través de la Paridad Descubierta de Intereses: si R€ baja y R$ permanece constante, los depósitos en euros resultan menos atractivos, los inversores venden euros para comprar dólares, y el euro se deprecia hasta que se restaura el equilibrio. A largo plazo, el efecto puede amplificarse —overshooting— o moderarse según las expectativas de inflación y los fundamentos macroeconómicos.`,
...
function genPPA(live){
  ...
  return {
    topicId:"t6_ppa",
    context:`La Paridad del Poder Adquisitivo (PPA) es la teoría dominante del tipo de cambio a largo plazo: establece que el poder de compra de una unidad monetaria debe ser equivalente en todos los países una vez convertida al tipo de cambio vigente. En su versión relativa —la más útil empíricamente—, predice que el tipo de cambio varía proporcionalmente al diferencial de inflación entre países. En este escenario, la Eurozona registra una inflación esperada del ${iE}% anual, mientras que en EE.UU. se proyecta un ${iU}% anual. Este diferencial de ${diff.toFixed(1)} puntos porcentuales sugiere que, según la PPA, el dólar debería ${iU>iE?"depreciarse":"apreciarse"} frente al euro a lo largo de los próximos ${anos} año${anos>1?"s":""}. La PPA funciona razonablemente bien en horizontes de 5-10 años, pero falla sistemáticamente a corto plazo debido a los bienes no comerciables, las barreras arancelarias y la predominancia de factores financieros en la determinación del tipo de cambio (Efecto Balassa-Samuelson, índice Big Mac de The Economist). Un analista de divisas de un banco de inversión utiliza la PPA para construir su previsión de largo plazo del EUR/USD.`,
...
function genTipoCambioReal(live){
  ...
  return {
    ...
    context:`El tipo de cambio nominal entre el euro y el dólar se sitúa actualmente en ${fmtN(eu,4)} €/$. Sin embargo, el tipo de cambio nominal por sí solo no dice nada sobre la competitividad real de las exportaciones europeas: lo que importa es cuántos bienes europeos hay que entregar a cambio de bienes americanos, es decir, el tipo de cambio real (TCR). El TCR corrige el tipo nominal por el diferencial de niveles de precios entre los dos países: si los precios en EE.UU. suben más deprisa que en la Eurozona —o si el euro se aprecia nominalmente—, los productos europeos se encarecen en términos relativos y la competitividad exterior se deteriora. Este mecanismo es clave para el análisis de la balanza por cuenta corriente: una apreciación real del euro presiona a la baja las exportaciones y estimula las importaciones, deteriorando el saldo comercial. Con una inflación media del ${iE}% en la Eurozona frente al ${iU}% en EE.UU. durante los últimos ${a} años, y una variación nominal del tipo de cambio del ${(dE*100).toFixed(2)}%, el economista del BCE analiza si la Eurozona ha ganado o perdido competitividad en términos reales.`,
